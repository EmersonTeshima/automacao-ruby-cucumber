...
Quando('clico no item do menu {string}') do |sobre_nos| 
    home.click_link(sobre_nos)
    sleep 2
end

Então('devo acessar o conteúdo {string}') do |verifica_sobre_nos|
    home.validate_text_menu_sobreNos(verifica_sobre_nos)
    sleep 2
end


Quando('clicar no menu {string}') do |depoimentos| 
...
Quando('acesso o item {string}') do |parceiros|
    home.click_link(parceiros) 
    sleep 2
end
Então('devo enxergar a tela {string}') do |verifica_parceiros|
    home.validate_text_menu_parceiros(verifica_parceiros)
    sleep 2
end

Quando('eu acesso o último item {string}') do |faleConosco| 
    home.click_link(faleConosco) 
     sleep 2
end
Então('devo acessar o título {string}') do |verifica_faleConosco|
